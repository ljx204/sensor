...
void STK832x_stkMotion_init(void ) 
{
	  ...
	  if( 0 == res )
    {
		...
		// reset and enable pedometer 重置并启用计步器
		stkMotion_Clear_Pedometer_Value(); 
		
		// level, default: 10 (1~50), window, default: 2 (range 0~4), 
		//window 0 is most sensitive 电平，默认值：10(1~50)，窗口，默认值：2(范围0~4)，窗口0是最敏感的
    stkMotion_Set_Pedometer_Params(10, 2); 
		
    // reset calorie / distance data 重置卡路里/距离数据
		stkMotion_Clear_Step_Calorie_Distance_Value(); 
		
		//monitor time: 59min (range 1~59min), step: 30 监控时间：59min(范围1~59min)，步长：30
    stkMotion_Set_Sedentary_Parma(3, 30); 
		
		//height: 180cm, weight: 70kg, age: 30 years old, gender: male 身高：180厘米，体重：70公斤，年龄：30岁，性别：男性
		stkMotion_Set_Calorie_Parma(180, 70, 30, 1); 
		
		// dir is 6 (0~7), sensitivity is 5 (1~5) ( 5 is most sensitive) DIR为6(0~7)，灵敏度为5(1~5)(5为最敏感)
		stkMotion_Set_Raise_Parma(6, 5); 
		
		// sensitivity is 5 (1~10), level 1 is most sensitive 灵敏度为5(1~10)，级别1最敏感
   ...
   // the slope threshold of shake, unit: mg. default is 80mg (75 ~ 250mg) 
	 // 抖动的斜率阈值，单位：mg。默认值为80毫克(75~250毫克)
